  ...
  export default {
    ...
    methods: {
      ...
      initUI(newSpell) {
        this.getFontData('')
        let self = this
        api.getSpellDetail(newSpell, this.currMaxPage, function (data) {
      ...
      initListScroll() {
        ...
        if (this.$refs.listWrapper) {
          this.spellScroll = new IScroll(this.$refs.listWrapper, {
            click: true,
            probeType: 3
          })
          this.spellScroll.on('scroll', function () {
          ...
          this.spellScroll.on('scrollEnd', function () {
            if (self.nextPage) {
              self.loadNewSpellList()
              self.spellScroll.scrollTo(0, this.y - 100)
              self.nextPage = false
            }
            self.hideGetMoreDiv()
            self.hideNoMoreDiv()
    ...
    computed: {
      ...mapGetters({
        spell: 'newSpell',
        fontList: 'fontListBySpell',
        isShowList: 'isShowList',
        currMaxPage: 'spellCurrMaxPage',
        listLen: 'spellListLen'
      })
    },
    created() {
      this.$nextTick(() => {
